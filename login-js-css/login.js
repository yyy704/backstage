	...
	function checkForm() {
	    ...
	    //验证规则汇总
	    // var id = new RegExp(/[0-9]{18}/); //匹配用户名
	    var password = new RegExp(/\w{6,16}/); //字母6到16次，用于密码匹配
	    // var chinese = new RegExp(/[^\u4e00-\u9fa5]|^$/); //匹配中文以外的字符，用于真实姓名验证
	    var chinese = new RegExp(/./); //万能匹配
	    // var chinese = new RegExp(/([\u4e00-\u9fa5]\[]0-9\w)+/); //匹配中文以外的字符，用于用户名验证
	    var email = new RegExp(/\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/); //用于邮箱格式验证

	    //用户名username验证(同样适用身份证验证)
	    if (form.username != undefined && !chinese.test(form.username.value)) {
	        //不符合匹配规则
	        onClick($(form.username)[0], '用户名不规范');
	        //阻止页面跳转
	        return false;
	    };
	    ...
	    //注册确认密码password验证
	    if (form.repassword != undefined && form.password.value !== form.repassword.value) {
	        console.log('aa');
	        //不符合匹配规则
	        onClick($(form.repassword)[0], '两次密码不一致');
	        //阻止页面跳转
	        return false;
	    };
	    ...
	    if (form.realname != undefined && chinese.test(form.realname.value)) {
	        //有非中文字符
	        onClick($(form.realname)[0], '用户名不规范');
	        //阻止页面跳转
	        return false;
	    };
